                                    ...
                                    <table id="users-table" role="grid" class="gridjs-table" style="height: auto; text-align: center;">
                                        <thead class="gridjs-thead">
                                            <tr class="gridjs-tr">
                                                ...
                                                <th data-column-id="email" class="gridjs-th gridjs-th-sort" tabindex="0" style="width: 220px;">
                                                    <div class="gridjs-th-content">Email</div>
                                                </th>
                                                <th data-column-id="role" class="gridjs-th gridjs-th-sort" tabindex="0" style="width: 180px;">
                                        ...
                                        <tbody class="gridjs-tbody">
                                            ...
                                            <?php foreach ($users as $index => $user): ?>
                                                <tr class="gridjs-tr align-middle">
                                                    ...
                                                    <td data-column-id="name" class="gridjs-td"><?= esc($user['username']) ?></td>
                                                    <td data-column-id="email" class="gridjs-td"><?= esc($user['email']) ?></td>                                                
                                                    <td data-column-id="role" class="gridjs-td"><?= esc($user['role_name']) ?></td>                                                    
                                                    <!-- <td data-column-id="actions" class="gridjs-td">
                                                        <div class="d-flex flex-wrap justify-content-start">
                                                            <a href="<?= base_url('/user/edit/' . $user['id']) ?>" title="Edit" style="font-size: 20px;">
                                                                <i class="ri-file-edit-line"></i>
                                                            </a>
                                                            <form action="<?= site_url('/delete/' . $user['id']) ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this user?');">
                                                                <?= csrf_field() ?>
